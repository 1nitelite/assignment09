...
NToulas, 2022-Jun-28, Completed TODOs
"""
...
class FileIO:
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_track, 'r') as disc:
                for track in disc:
                    track_data = track.strip().split(',')
                    track_row = DC.Track(int(track_data[0]), track_data[1], track_data[2])
                    lst_Tracks.append(track_row)
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
